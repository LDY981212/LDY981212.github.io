...
const projectItems: ProjectItems[] = [
  ...
  },
  {
    id: "6",
    title: "Webinow",
    created: "2025.12 ~ 2026.07 (웨이온 · 4人 팀 프로젝트)",
    subTitle:
    ...
    solution: [
      {
        ...
        situation:
          "동시에 여러 영상을 렌더링할 때 외부 렌더링 API(Shotstack)의 초당 요청 한도를 넘어 429(Too Many Requests) 응답이 발생하고, 동시에 DB 커넥션도 부족해지는 문제가 있었습니다.",
        task: "요청 한도 안에서 렌더링을 안정적으로 처리하고, 동시 작업 시 자원이 고갈되지 않게 하는 것입니다.",
        result:
          "요청 한도를 넘지 않고 렌더링이 재시도를 통해 완료되었고, 동시 작업이 많아도 DB 커넥션이 고갈되지 않았습니다.",
        action:
          "Shotstack 요청에 초당 요청 수(RPS) 리미터와 429 발생 시 지수 백오프 재시도를 적용하고, DB 커넥션 예산을 명시하며 사용자별 동시 작업 수를 제한하는 가드를 두었습니다.",
      },
      {
        head: "음성 인식 모델 파라미터 변경으로 요청이 실패하는 문제점",
        situation:
          "Deepgram 음성 인식 모델을 nova-3로 올리면서, 기존에 쓰던 keywords 파라미터가 더 이상 지원되지 않아 400(INVALID_QUERY_PARAMETER) 오류로 인식 요청이 실패했습니다.",
        task: "새 모델에서도 도메인 키워드 부스트가 동작하도록 인식 요청을 맞추는 것입니다.",
        result:
          "새 모델에서도 오류 없이 음성 인식이 동작하고, 도메인 단어 인식 정확도도 유지되었습니다.",
        action:
          "모델 버전에 따라 keywords와 keyterm 파라미터를 분기해서 보내도록 처리해, 구·신 모델 모두에서 키워드 부스트가 동작하게 했습니다.",
      },
    ],
  },
];
